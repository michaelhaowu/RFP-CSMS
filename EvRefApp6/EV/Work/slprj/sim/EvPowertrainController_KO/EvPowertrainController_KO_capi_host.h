#include "__cf_EvPowertrainController_KO.h"
#ifndef RTW_HEADER_EvPowertrainController_KO_cap_host_h_
#define RTW_HEADER_EvPowertrainController_KO_cap_host_h_
#ifdef HOST_CAPI_BUILD
#include "rtw_capi.h"
#include "rtw_modelmap.h"
typedef struct { rtwCAPI_ModelMappingInfo mmi ; }
EvPowertrainController_KO_host_DataMapInfo_T ;
#ifdef __cplusplus
extern "C" {
#endif
void EvPowertrainController_KO_host_InitializeDataMapInfo (
EvPowertrainController_KO_host_DataMapInfo_T * dataMap , const char * path )
;
#ifdef __cplusplus
}
#endif
#endif
#endif
